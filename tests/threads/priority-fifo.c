...
test_priority_fifo (void) 
{
  ...
  ASSERT (thread_get_priority () == PRIO_DEFAULT);
  ...
  thread_set_priority (PRIO_DEFAULT + 2);
  for (i = 0; i < THREAD_CNT; i++) 
    {
      ...
      d->lock = &lock;
      d->op = &op;
      thread_create (name, PRIO_DEFAULT + 1, simple_thread_func, d);
    }

  thread_set_priority (PRIO_DEFAULT);
